...
function PostItem({post, handleLike}: PostItemProps) {
    ...
    return (
        <section className="px-5 py-3 border-b border-lineBg  flex flex-col gap-2 text-white" key={post._id}>
            <header className="flex flex-row items-center">
                <UserCircle size={40} weight='light' fill="" />
                <Heading size="xs" className="ml-2">{post.profile.name}</Heading>
                {/*<Heading  className="ml-2 text-sm">{`@${post.profile.user}`}</Heading>*/}
            </header>

            <Text asChild size="md" className="pl-2 ml-8 flex flex-col gap-2">
            ...
            {post.image ? (
                ...
                <Text asChild size="sm" className="pl-2 ml-8">
